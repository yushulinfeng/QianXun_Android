package com.szdd.qianxun.tools.views.slidemenu;
...
import com.szdd.qianxun.tools.views.slidemenu.SlidingMenu.OnClosedListener;
import com.szdd.qianxun.tools.views.slidemenu.SlidingMenu.OnOpenedListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	// 是否缩放界面
	private boolean needScare = false;
	 ...
	 * 设置触摸的模式
	 */
	public void setScareEnable(boolean b) {
		needScare = b;
	}
	...
	public void scrollTo(int x, int y) {
		...
		if (!needScare)
			return;
		int screen_width = ((WindowManager) getContext().getSystemService(
				Context.WINDOW_SERVICE)).getDefaultDisplay().getWidth();
		final float max_scare = screen_width - mViewBehind.getWidthOffset();// 侧滑宽度
		float scare = ((x < -max_scare) ? -max_scare : x) / (-max_scare);
		ViewHelper.setPivotX(mViewBehind, 0);
		ViewHelper.setPivotY(mViewBehind, mViewBehind.getHeight() / 2);
		ViewHelper.setScaleX(mViewBehind, 0.6f + 0.4f * scare);
		ViewHelper.setScaleY(mViewBehind, 0.6f + 0.4f * scare);

		scare = 1.0f - (((x < -max_scare) ? -max_scare : x) / (-max_scare));
		ViewHelper.setPivotX(mContent, 0);
		ViewHelper.setPivotY(mContent, mContent.getHeight() / 2);
		ViewHelper.setScaleX(mContent, 0.8f + 0.2f * scare);
		ViewHelper.setScaleY(mContent, 0.8f + 0.2f * scare);
		// Log.e("AAAAA", "AAAAA  " + x + "--" + scare);
	}
